...
执行摘要必须包含 template ID、输入来源、行数或任务规模、将要执行的动作、预计费用以及明确提示：`回复“确认提交”后我才会开始执行`。
...
用户询问控制台，或 Agent 成功提交、执行、回填批处理任务后，应提示当前控制台入口：

- 控制台页面：`https://loomloom.shengsuanyun.com/home/workflow-runs`
- 根地址：`https://loomloom.shengsuanyun.com/`
- Batch API：`https://loomloom.shengsuanyun.com/batch`

如果用户尚未登录，提示先通过胜算云主站登录，再返回控制台：
