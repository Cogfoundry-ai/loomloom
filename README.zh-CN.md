...
# 2. 估算成本（输入文件包含按 listing schema 组织的 taskInputs 数组）
...
使用 listing detail 理解公开字段并获得 `listingVersionId`，但不要从 `inputSchemaSnapshot` 推断内部 step ID：公开快照可能不会暴露这些信息。当前 CLI 需要精确的 Product API `taskInputs` payload。如果用户或集成方没有提供对应映射，请停下来索要兼容的 request JSON，不要猜测。
...
对于 `template submit-file`、`template-spec submit-workbook`、`run submit`、`template-spec run` 和 `market run`，请传入明确的 `--client-request-id`，随请求保存它，并且只在重试完全相同 payload 时复用。如果 payload 变化，请使用新 ID。对于付费或会改变远程状态的命令，不要在结果不明确时盲目重试；应先检查原请求是否已成功。
