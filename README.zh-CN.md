...
```text
帮我从 GitHub 安装 LoomLoom：https://github.com/Cogfoundry-ai/loomloom
我的服务器地址是 https://loomloom.cogfoundry.ai/loom/v1，token 是 <你的token>
装完跑 doctor 检查是否正常
```
...
`taskFixedFeeT`、`amountT` 等金额值使用 API units，其中 10,000,000 units 等于 1 个货币单位。Market 与账务相关命令（`market list/show/quote/run`、`market workbook quote/run`、`listing list/show/versions`、`usage list/get`、`creator transactions`）的默认文本输出会自动把这些金额换算为可读格式（例如 `CNY 0.5000000` 或 `USD 0.5000000`），同时保留原始 `*T` 值用于对账。`--output json` 始终返回后端原始 `*T` 字段，不做任何换算。
